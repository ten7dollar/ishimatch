...
/** ← 「どちらでも良い」ではなく「どちらでも」に統一 */
const EMERGENCY = ["二次救急", "三次救急", "どちらでも"] as const;
...
export default function StudentHospitalSearchPage() {
  ...
  const [startYears, setStartYears] = useState<Set<number>>(new Set()); // 将来用
  const [salary, setSalary] = useState<(typeof SALARY_BANDS)[number]>("問わない");
  const [bonus, setBonus] = useState<(typeof BONUS)[number]>("問わない");
  /** ← こちらも "どちらでも" を初期値に */
  const [emergency, setEmergency] = useState<(typeof EMERGENCY)[number]>("どちらでも");
  ...
  /* -------- Set 操作用ユーティリティ（未定義エラーの元だったので明示定義） -------- */
  ...
  const selectRegion = (r: { name: string; prefs: string[] }) =>
    setPrefSet(prev => {
      const next = new Set(prev);
      r.prefs.forEach(p => next.add(p));
      return next;
    });

  const clearRegion = (r: { name: string; prefs: string[] }) =>
    setPrefSet(prev => {
      const next = new Set(prev);
      r.prefs.forEach(p => next.delete(p));
      return next;
    });
  ...
  const load = async () => {
    setLoading(true);
    try {
      let q = supabase.from("hospitals").select("*", { count: "exact" }).limit(200);

      // 病院名：部分一致
      const qStr = nameDebounce.trim();
      if (qStr.length > 0) q = q.ilike("name", `%${escapeIlike(qStr)}%`);

      if (mode === "AND") {
        if (prefSet.size > 0) q = q.in("prefecture", Array.from(prefSet));
        if (emergency !== "どちらでも") q = q.eq("facility_type", emergency);
        ...
        if (emergency !== "どちらでも") orParts.push(`facility_type.eq.${emergency}`);
  ...
  useEffect(() => { load(); /* 初回/モード変更 */ }, [mode]);
  useEffect(() => { load(); /* 病院名入力のデバウンス */ }, [nameDebounce]);
      ...
      <div className="grid md:grid-cols-[300px_1fr] gap-4 md:gap-8">
        ...
        <aside className="card h-fit p-4 md:p-5 sticky top-4 self-start space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-3 text-sm">
              <label className="flex items-center gap-1"><input type="radio" checked={mode==="AND"} onChange={()=>setMode("AND")} />AND（すべて満たす）</label>
              <label className="flex items-center gap-1"><input type="radio" checked={mode==="OR"} onChange={()=>setMode("OR")} />OR（いずれかを満たす）</label>
            </div>
          </div>

          {/* 地域 */}
          <details open>
            <summary className="cursor-pointer text-sm font-semibold text-primary-700 mb-2 select-none">希望勤務地</summary>
                ...
                <div key={r.name} className="border rounded p-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button type="button" onClick={()=>selectRegion(r)} className="text-xs px-2 py-0.5 border rounded">全選択</button>
                      <button type="button" onClick={()=>clearRegion(r)} className="text-xs px-2 py-0.5 border rounded">解除</button>
                    </div>
                  </div>
          ...
          {/* 研修開始年度（将来用） */}
          <details>
            <summary className="cursor-pointer text-sm font-semibold text-primary-700 mb-2 select-none">研修開始年度</summary>
            <div className="flex flex-wrap gap-2">
              {START_YEARS.map(y => (
                <label key={y} className="flex items-center gap-1 text-sm">
                  <input type="checkbox" checked={startYears.has(y)} onChange={()=>setStartYears(prev => toggleSet(prev, y))} />
                  {y}年
                </label>
              ))}
            </div>
            <p className="text-[11px] text-gray-500 mt-1">※現状は表示に反映していません（DB側の項目追加後に有効化）</p>
          </details>

          {/* ラジオ群 */}
          <RadioGroup title="希望年収（初年度）" options={SALARY_BANDS} value={salary} onChange={setSalary} />
          ...
          <div className="pt-2 flex flex-col gap-2">
            ...
            <button type="button" onClick={clearAll} className="border rounded py-2 text-sm hover:bg-gray-50">条件をクリア</button>
          </div>
        </aside>
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <select className="border rounded px-3 py-1 text-sm" onChange={()=>{}}>
          ...
          {rows.map(h => (
            <div key={h.id} className="rounded-xl border bg-white p-4 hover:shadow-sm transition">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-2 text-[11px] md:text-xs">
                    ...
                    {h.bonus === "あり" && <span className="px-2 py-0.5 rounded-full bg-gray-50 text-gray-700">賞与あり</span>}
                  </div>
                </div>

                <div className="flex gap-2 shrink-0">
                  <Link href={`/student/apply?hospitalId=${encodeURIComponent(h.id)}`} className="px-3 py-1.5 rounded bg-primary-600 text-white text-sm hover:bg-primary-700">初回面談を申し込む</Link>
                </div>
              </div>
              ...
              <div className="mt-3 flex justify-end gap-2">
                <Link href={`/student/hospitals/${encodeURIComponent(h.id)}`} className="px-3 py-1.5 rounded border text-sm hover:bg-gray-50">詳細を見る</Link>
...
/* ---------------------------
   小さな部品/ユーティリティ
--------------------------- */
function RadioGroup<T extends string>({ title, options, value, onChange }: { title:string; options: readonly T[]; value:T; onChange:(v:T)=>void }) {
  return (
    <details open>
      <summary className="cursor-pointer text-sm font-semibold text-primary-700 mb-2 select-none">{title}</summary>
      <div className="flex flex-wrap gap-2">
        {options.map(opt => (
...
/** in句用：("A","B") */
function toInList(arr: string[]) { return `(${arr.map(v => `"${v}"`).join(",")})`; }
/** デバウンス */
...
function FavButton({ hospitalId }: { hospitalId: string }) {
  ...
  return (
    <button onClick={toggle} className={`px-3 py-1 text-sm rounded border ${active ? "bg-red-50 text-red-600 border-red-300" : ""}`}>
      {active ? "★ 検討中" : "☆ 検討に追加"}
    </button>
  );
}