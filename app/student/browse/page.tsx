  ...
  const load = async () => {
    ...
    try {
      ...
      if (mode === "AND") {
        ...
        switch (nights) {
          case "〜2回":
            q = q.eq("duty_frequency", "~2回");
            break;
          case "3〜4回":
            q = q.eq("duty_frequency", "3~4回");
            break;
          case "5回〜":
            q = q.eq("duty_frequency", "5回以上");
            break;
        }
        ...
        switch (interns) {
          case "〜5人":
            q = q.lte("residents_first_year", 5);
            break;
          case "6〜10人":
            q = q.gte("residents_first_year", 6).lte("residents_first_year", 10);
            break;
          case "11〜20人":
            q = q.gte("residents_first_year", 11).lte("residents_first_year", 20);
            break;
          case "21人〜":
            q = q.gte("residents_first_year", 21);
            break;
        }

        switch (beds) {
          case "〜100床":
            q = q.lte("bed_count", 100);
            break;
          case "100〜300床":
            q = q.gte("bed_count", 100).lte("bed_count", 300);
            break;
          case "300〜499床":
            q = q.gte("bed_count", 300).lte("bed_count", 499);
            break;
          case "500床〜":
            q = q.gte("bed_count", 500);
            break;
        }

        if (bonus !== "問わない") q = q.eq("bonus", bonus);
      } else {
        // OR モード：どれか1つ当てはまればOK（PostgREST の or(...) 構文）
        ...
        switch (nights) {
          case "〜2回":
            orParts.push(`duty_frequency.eq.~2回`);
            break;
          case "3〜4回":
            orParts.push(`duty_frequency.eq.3~4回`);
            break;
          case "5回〜":
            orParts.push(`duty_frequency.eq.5回以上`);
            break;
        }

        switch (salary) {
          case "〜400万円":
            orParts.push(`salary_1st_year_min.lte.400`);
            break;
          case "400〜599万円":
            orParts.push(`and(salary_1st_year_min.gte.400,salary_1st_year_min.lte.599)`);
            break;
          case "600〜799万円":
            orParts.push(`and(salary_1st_year_min.gte.600,salary_1st_year_min.lte.799)`);
            break;
          case "800万円以上":
            orParts.push(`salary_1st_year_min.gte.800`);
            break;
        }

        switch (interns) {
          case "〜5人":
            orParts.push(`residents_first_year.lte.5`);
            break;
          case "6〜10人":
            orParts.push(`and(residents_first_year.gte.6,residents_first_year.lte.10)`);
            break;
          case "11〜20人":
            orParts.push(`and(residents_first_year.gte.11,residents_first_year.lte.20)`);
            break;
          case "21人〜":
            orParts.push(`residents_first_year.gte.21`);
            break;
        }

        switch (beds) {
          case "〜100床":
            orParts.push(`bed_count.lte.100`);
            break;
          case "100〜300床":
            orParts.push(`and(bed_count.gte.100,bed_count.lte.300)`);
            break;
          case "300〜499床":
            orParts.push(`and(bed_count.gte.300,bed_count.lte.499)`);
            break;
          case "500床〜":
            orParts.push(`bed_count.gte.500`);
            break;
        }
    ...
    <main className="max-w-6xl mx-auto px-4 md:px-8 py-6 space-y-6">
      ...
      <div className="card p-3 md:p-4">
        ...
        <input
          ...
          placeholder="病院名で検索（例「赤十字」「順天堂」など）"
        />
      </div>
