...
function seedOnce() {
  ...
  const iso = (y: number, m: number, d: number) => new Date(y, m - 1, d, 10, 0).toISOString();
  const mock: Application[] = [
    { id: "a-001", studentId: "st-001", studentName: "山田太郎", university: "東京大学医学部",  gradYear: 2026, appliedAt: iso(2025,11,7), status: "新規",     email: "yamada@example.com" },
    { id: "a-002", studentId: "st-002", studentName: "佐藤花子", university: "京都大学医学部",  gradYear: 2026, appliedAt: iso(2025,11,5), status: "面談済み", email: "sato@example.com" },
    { id: "a-003", studentId: "st-003", studentName: "田中美咲", university: "慶應義塾大学医学部", gradYear: 2026, appliedAt: iso(2025,11,6), status: "選考中",   email: "tanaka@example.com" },
  ];
...
export default function HospitalApplicationsPage() {
  ...
  const [gradFilter, setGradFilter] = useState<"すべて" | 2025 | 2026 | 2027 | 2028>("すべて");
  ...
  const { total, byStatus } = useMemo(() => {
    const t = rows.length;
    const map: Record<AppStatus, number> = { 新規:0, 面談済み:0, 見学予定:0, 選考中:0, 内定:0, 辞退:0 };
      ...
      <section className="rounded-xl border bg-white p-4">
        <div className="grid md:grid-cols-3 gap-3">
          <div>
            ...
            <select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value as any)} className="border rounded px-3 py-2 text-sm w-full">
          ...
          <div>
            ...
            <select value={gradFilter} onChange={(e)=>setGradFilter(e.target.value === "すべて" ? "すべて" : Number(e.target.value) as any)} className="border rounded px-3 py-2 text-sm w-full">
              <option value="すべて">すべて</option>
          ...
          <div>
            ...
            <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="名前・大学・メールで検索" className="border rounded px-3 py-2 text-sm w-full" />
          </div>
        </div>
      </section>
            ...
            <tbody>
              {list.map(r => (
                <tr key={r.id} className="border-b last:border-0 hover:bg-primary-50">
                  ...
                  <td className="py-2 px-4">{r.university}・{r.gradYear}年卒</td>
                  ...
                  <td className="py-2 px-4">
                    <select value={r.status} onChange={(e)=>updateStatus(r.id, e.target.value as AppStatus)} className="border rounded px-2 py-1 text-sm">
                      {ALL_STATUS.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </td>
                  {/* 操作列の <td> は削除 */}
