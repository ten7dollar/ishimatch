...
type DocRow = {
  id: string; title: string; path: string; file_name: string;
...
export default function StudentDocuments({ studentId }: { studentId: string }) {
  ...
  const onView = async (row: DocRow) => {
    try {
      setBusy(true);
      const resp = await fetch("/api/documents/sign", {
        method: "POST",
  ...
  return (
    <section className="card p-4 space-y-3">
      ...
      <ul className="divide-y border rounded">
        {list.map((row)=>(
          <li key={row.id} className="flex items-center justify-between px-3 py-2">
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500">{row.file_name} / {row.mime_type ?? "?"}</p>
            </div>
            <button
        ...
        {list.length === 0 && <li className="px-3 py-6 text-sm text-gray-500 text-center">まだ提出がありません</li>}
      </ul>
    </section>
  );
}